...
- Preliminary results for NAIIS survey was made available in all but 10 states.
   - MISSING: Benue, Borno, Delta, Ebonyi, Edo, Katsina, Oyo, Plateau, Yobe, Zamfara
...
#unmet need
  ...
  viz_burden %>% 
    ...
    coord_flip() +
    labs(x = "", y = "PLHIV", caption = "state bars ordered on unmet need (blue)") +
    theme_bw() +
    theme(text = element_text(size = 18),
          axis.title = element_text(color = main_gray),
          axis.ticks = element_blank(),
...
```{r burden_share}
  ...
  viz_burden %>% 
      ...
      coord_flip() +
      labs(x = "", y = "Share of PLHIV", caption = "state bars ordered on unmet need (blue)") +
      theme_bw() +
      theme(text = element_text(size = 18),
            axis.title = element_text(color = main_gray),
            axis.ticks = element_blank(),
...
## The estimated cascade shows low 1st 90, but sucessful 2nd and 3rd 90.
...
## ...and actual we see higher achievement for males in many districts.
...
## Comparing program results to NAIIS, Rivers and Anambra are heavily under reporting in MER compared with FCT, Adamawa, and Niger whihc are heavily under reporting.
...
```{r tx_curr}
  ...
  reporting <- left_join(tx_curr_est, tx_curr, by = c("state", "sex")) %>% 
    filter(sex == "total") %>% 
    mutate(reporting = ifelse(Diagnosed > TX_CURR, "MER total under reported", "MER total over reported")) %>% 
    select(state, reporting)

  viz_tx_curr_comp <- left_join(tx_curr_est, tx_curr, by = c("state", "sex")) %>% 
    left_join(., reporting, by = "state") %>% 
    gather(type, val, -state, -sex, -reporting) %>% 
    arrange(state, sex, type) %>% 
    mutate(sex = str_to_title(sex),
           sex = factor(sex, c("Total", "Female", "Male")),
           lab = case_when(state == "Kaduna" & type == "TX_CURR" & sex == "Total" ~ "MER",
                           state == "Kaduna"& type == "Diagnosed" & sex == "Total" ~ "NAIIS"))
  ...
  viz_tx_curr_comp %>%
    ...
    geom_point(color = ifelse(viz_tx_curr_comp$type == "TX_CURR", comp_gray, main_blue),
               size = 5, na.rm = TRUE) +
    geom_text(aes(label = lab),
              hjust = ifelse(viz_tx_curr_comp$type == "TX_CURR", -.5, 1.5), #vjust = -1,
              size = 5,
    ...
    expand_limits(y = 0) +
    labs(x = "", y  = "") +
    facet_grid(reporting ~ sex, scales = "free_y") +
    theme_bw() +
    theme(text = element_text(size = 18),
          legend.position = "none",
          axis.title = element_text(color = main_gray),
